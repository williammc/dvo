...
void OptimizedSelfAdjointMatrix6x6f::rankUpdate(
    const Eigen::Matrix<float, 6, 1> &u, const float &alpha) {
...
void OptimizedSelfAdjointMatrix6x6f::rankUpdate(
    const Eigen::Matrix<float, 2, 6> &v, const Eigen::Matrix2f &alpha) {
  /**
   * layout of alpha:
   *
   *   1 2
   *   3 4
   */
  __m128 alpha1324 = _mm_load_ps(
      alpha.data()); // load first two columns from column major data
  ...
  __m128 u1a2a1b2b = _mm_hadd_ps(_mm_mul_ps(v1a1b2a2b, alpha1313),
                                 _mm_mul_ps(v1a1b2a2b, alpha2424));

  __m128 u1a1b1a1b =
      _mm_shuffle_ps(u1a2a1b2b, u1a2a1b2b, _MM_SHUFFLE(2, 0, 2, 0));
  __m128 u2a2b2a2b =
      _mm_shuffle_ps(u1a2a1b2b, u1a2a1b2b, _MM_SHUFFLE(3, 1, 3, 1));

  // upper left 2x2 block of A matrix in row major format
  __m128 b11 = _mm_hadd_ps(_mm_mul_ps(u1a1b1a1b, v1a1b2a2b),
  ...
  __m128 b12 = _mm_hadd_ps(_mm_mul_ps(u1a1b1a1b, v3a3b4a4b),
  ...
  __m128 b13 = _mm_hadd_ps(_mm_mul_ps(u1a1b1a1b, v5a5b6a6b),
                           _mm_mul_ps(u2a2b2a2b, v5a5b6a6b));
  _mm_store_ps(data + 8, _mm_add_ps(_mm_load_ps(data + 8), b13));

  __m128 u3a4a3b4b = _mm_hadd_ps(_mm_mul_ps(v3a3b4a4b, alpha1313),
                                 _mm_mul_ps(v3a3b4a4b, alpha2424));

  __m128 u3a3b3a3b =
      _mm_shuffle_ps(u3a4a3b4b, u3a4a3b4b, _MM_SHUFFLE(2, 0, 2, 0));
  __m128 u4a4b4a4b =
      _mm_shuffle_ps(u3a4a3b4b, u3a4a3b4b, _MM_SHUFFLE(3, 1, 3, 1));

  // center center 2x2 block of A matrix in row major format
  __m128 b22 = _mm_hadd_ps(_mm_mul_ps(u3a3b3a3b, v3a3b4a4b),
  ...
  __m128 b23 = _mm_hadd_ps(_mm_mul_ps(u3a3b3a3b, v5a5b6a6b),
                           _mm_mul_ps(u4a4b4a4b, v5a5b6a6b));
  _mm_store_ps(data + 16, _mm_add_ps(_mm_load_ps(data + 16), b23));

  __m128 u5a6a5b6b = _mm_hadd_ps(_mm_mul_ps(v5a5b6a6b, alpha1313),
                                 _mm_mul_ps(v5a5b6a6b, alpha2424));

  __m128 u5a5b5a5b =
      _mm_shuffle_ps(u5a6a5b6b, u5a6a5b6b, _MM_SHUFFLE(2, 0, 2, 0));
  __m128 u6a6b6a6b =
      _mm_shuffle_ps(u5a6a5b6b, u5a6a5b6b, _MM_SHUFFLE(3, 1, 3, 1));

  // bottom right 2x2 block of A matrix in row major format
  __m128 b33 = _mm_hadd_ps(_mm_mul_ps(u5a5b5a5b, v5a5b6a6b),
...
template <>
void MathSse<Sse::Enabled, float>::addOuterProduct(
    Eigen::Matrix<float, 6, 6> &mat, const Eigen::Matrix<float, 6, 1> &vec,
    const float &scale) {
  /**
   * idea:
   *
   ...
   *
   * the vectors for multiplication can be obtained by shuffling v1 and v2 (they
   *are prescaled with scale to v1s and v2s)
   *
  ...
  _mm_store_ps(target_ptr,
               _mm_add_ps(_mm_load_ps(target_ptr), _mm_mul_ps(fac, v1)));
  target_ptr += 4;

  fac = _mm_shuffle_ps(v1s, v1s, _MM_SHUFFLE(1, 1, 0, 0));
  _mm_store_ps(target_ptr,
               _mm_add_ps(_mm_load_ps(target_ptr), _mm_mul_ps(fac, v2)));
  target_ptr += 4;

  fac = _mm_shuffle_ps(v1s, v1s, _MM_SHUFFLE(1, 1, 1, 1));
  _mm_store_ps(target_ptr,
               _mm_add_ps(_mm_load_ps(target_ptr), _mm_mul_ps(fac, v3)));
  target_ptr += 4;
...
template <>
void MathSse<Sse::Enabled, double>::addOuterProduct(
    Eigen::Matrix<double, 6, 6> &mat, const Eigen::Matrix<double, 6, 1> &vec,
    const double &scale) {
  ...
  _mm_store_pd(target_ptr,
               _mm_add_pd(_mm_load_pd(target_ptr), _mm_mul_pd(fac, v1)));
  target_ptr += 2;
  _mm_store_pd(target_ptr,
               _mm_add_pd(_mm_load_pd(target_ptr), _mm_mul_pd(fac, v2)));
  target_ptr += 2;
  _mm_store_pd(target_ptr,
               _mm_add_pd(_mm_load_pd(target_ptr), _mm_mul_pd(fac, v3)));
  target_ptr += 2;
...
template <>
void MathSse<Sse::Enabled, float>::add(Eigen::Matrix<float, 6, 1> &vec,
  ...
  _mm_store_ps(vec.data(),
               _mm_add_ps(_mm_load_ps(vec.data()), _mm_mul_ps(v1, s)));

  _mm_storel_pi((__m64 *)(vec.data() + 4),
                _mm_add_ps(_mm_loadl_pi(s, (__m64 *)(vec.data() + 4)),
                           _mm_mul_ps(v2, s)));
}

template <>
void MathSse<Sse::Enabled, double>::add(
    Eigen::Matrix<double, 6, 1> &vec, const Eigen::Matrix<double, 6, 1> &other,
    const double &scale) {
  __m128d s = _mm_set1_pd(scale);
  double *target_ptr = vec.data();

  _mm_store_pd(target_ptr,
               _mm_add_pd(_mm_load_pd(target_ptr),
                          _mm_mul_pd(s, _mm_load_pd(other.data() + 0))));
  target_ptr += 2;
  _mm_store_pd(target_ptr,
               _mm_add_pd(_mm_load_pd(target_ptr),
                          _mm_mul_pd(s, _mm_load_pd(other.data() + 2))));
  target_ptr += 2;
  _mm_store_pd(target_ptr,
               _mm_add_pd(_mm_load_pd(target_ptr),
                          _mm_mul_pd(s, _mm_load_pd(other.data() + 4))));
  target_ptr += 2;
}
