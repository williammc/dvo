...
void TDistributionScaleEstimator::configure(const float &param) { dof = param; }
  ...
  do {
    ...
    for (size_t idx = 0; idx < errors.size().area(); ++idx, ++data_ptr) {
      ...
      if (std::isfinite(data)) {
        num += 1.0f;
        lambda +=
            data * data * ((dof + 1.0f) / (dof + initial_lamda * data * data));
      }
    }
...
struct TDistributionScaleReduction {
  TDistributionScaleReduction(const float *data, const float initial_lambda,
                              const float dof)
      : data(data), initial_lambda(initial_lambda), dof(dof), lambda(0.0f),
        num(0) {}

  TDistributionScaleReduction(TDistributionScaleReduction &other, tbb::split)
      : data(other.data), initial_lambda(other.initial_lambda), dof(other.dof),
        lambda(0.0f), num(0) {}
  ...
  void operator()(const tbb::blocked_range<size_t> &r) {
    ...
    //  lambda4 = _mm_add_ps(lambda4, _mm_and_ps(isnan, _mm_mul_ps(data4square,
    //  _mm_div_ps(_mm_add_ps(dof4, ones),  _mm_add_ps(dof4, _mm_mul_ps(ini4,
    //  data4square))))));
    //
    //  //const float& data = *data_ptr;
    //  //
    ...
    //  //{
    //  //  tmp_num += 1;
    //  //  tmp_lambda += data * data * ( (dof + 1.0f) / (dof + initial_lambda *
    //  data * data) );
    //  //}
    //}
    ...
    // tmplambda += tmplambda4[0] + tmplambda4[1] + tmplambda4[2]+
    // tmplambda4[3];
    ...
    for (size_t idx = r.begin(); idx != r.end(); ++idx, ++data_ptr) {
      ...
      if (std::isfinite(data)) {
        tmpnum += 1.0f;
        tmplambda +=
            data * data * ((dof + 1.0f) / (dof + initial_lambda * data * data));
      }
    }
...
OptimizedTDistributionScaleEstimator::OptimizedTDistributionScaleEstimator(
    const float dof)
    : TDistributionScaleEstimator(dof) {}

float OptimizedTDistributionScaleEstimator::compute(
    const cv::Mat &errors) const {
  ...
  do {
    ...
    tbb::parallel_reduce(tbb::blocked_range<size_t>(0, errors.size().area()),
                         body);
...
ApproximateTDistributionScaleEstimator::ApproximateTDistributionScaleEstimator(
    const float dof)
    : TDistributionScaleEstimator(INITIAL_SIGMA) {}

float ApproximateTDistributionScaleEstimator::compute(
    const cv::Mat &errors) const {
...
TukeyInfluenceFunction::TukeyInfluenceFunction(const float b) { configure(b); }
...
TDistributionInfluenceFunction::TDistributionInfluenceFunction(
    const float dof) {
  configure(dof);
}
...
HuberInfluenceFunction::HuberInfluenceFunction(const float k) { configure(k); }
...
void HuberInfluenceFunction::configure(const float &param) { k = param; }
...
void WeightCalculation::calculateWeights(const cv::Mat &errors,
                                         cv::Mat &weights) {
  ...
  for (size_t idx = 0; idx < errors.size().area();
       ++idx, ++err_ptr, ++weight_ptr) {
