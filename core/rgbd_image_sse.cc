...
void convertRawDepthImageSse(const cv::Mat &input, cv::Mat &output,
                             float scale) {
  ...
  for (int idx = 0; idx < input.size().area();
...
static inline void dump(const char *prefix, __m128 v) {
  ...
  std::cerr << prefix << " " << data[0] << " " << data[1] << " " << data[2]
            << " " << data[3] << std::endl;
}
...
static inline __m128 interpolateBilinearWithDepthBufferSse(
    const cv::Mat &intensity, const cv::Mat &depth, const __m128 &xyxy_proj,
    const __m128 &zzzz, const __m128 &img_upper_bound) {
  ALIGN int address[4];
  ...
  x1wy1w = _mm_and_ps(inimage, x1wy1w); // [x1weight if x1 < width else 0,
                                        // y1weight if y1 < width else 0, ?, ?]

  // weights
  __m128 tmp = _mm_shuffle_ps(x0wy0w, x1wy1w, _MM_SHUFFLE(1, 0, 1, 0));

  __m128 x0101w = _mm_shuffle_ps(
      tmp, tmp,
      _MM_SHUFFLE(2, 0, 2, 0)); // [x0_weight, x1_weight, x0_weight, x1_weight]
  __m128 y0011w = _mm_shuffle_ps(
      tmp, tmp,
      _MM_SHUFFLE(3, 3, 1, 1)); // [y0_weight, y0_weight, y1_weight, y1_weight]
  __m128 w = _mm_mul_ps(x0101w, y0011w); // [x0_weight * y0_weight, x1_weight *
                                         // y0_weight, x0_weight * y1_weight,
                                         // x1_weight * y1_weight]
  ...
  // w = _mm_and_ps(_mm_and_ps(_mm_cmpord_ps(p_depth, p_depth),
  // _mm_cmpgt_ps(p_depth, zeps)), w);
  ...
  return _mm_or_ps(_mm_and_ps(notzero, _mm_div_ps(iw, wi)),
                   _mm_andnot_ps(notzero, NANS));
}

template <int PointCloudOption>
void RgbdImage::warpIntensitySseImpl(const AffineTransform &transformationx,
                                     const PointCloud &reference_pointcloud,
                                     const Eigen::Matrix3d &projection,
                                     RgbdImage &result,
                                     PointCloud &transformed_pointcloud) {
  // prepare transformation
  Eigen::Transform<float, 3, Eigen::Affine, Eigen::RowMajor> transformation =
      transformationx.cast<float>();
  ...
  if (PointCloudOption == WithPointCloud) {
    transformed_pointcloud.resize(Eigen::NoChange, width_ * height_);
  }
  ...
  for (size_t y = 0; y < height_; ++y) {
    for (size_t x = 0; x < width_; x += 2, warped_intensity_ptr += 2,
                warped_depth_ptr += 2, points += 8, tpoints += 8) {
      point1 = _mm_load_ps(points + 0);
      point2 = _mm_load_ps(points + 4);

      // matrix multiply and first horizontal add
      xy = _mm_hadd_ps(_mm_mul_ps(t1, point1),
                       _mm_mul_ps(t2, point1)); // [x0+1, x2+3, y0+1, y2+3]
      uv = _mm_hadd_ps(_mm_mul_ps(t1, point2),
                       _mm_mul_ps(t2, point2)); // [u0+1, u2+3, v0+1, v2+3]
      zw = _mm_hadd_ps(_mm_mul_ps(t3, point1),
                       _mm_mul_ps(t3, point2)); // [z0+1, z2+3, w0+1, w2+3]
      ...
      zzww = _mm_unpacklo_ps(zwzw,
                             zwzw); // [z0+1+2+3, z0+1+2+3, w0+1+2+3, w0+1+2+3]
      zzww_rcp = _mm_rcp_ps(zzww);  // 1 / zzww

      // projection
      xyuv_proj =
          _mm_add_ps(_mm_mul_ps(_mm_mul_ps(xyuv, fxyxy), zzww_rcp), oxyxy);

      // check image bounds
      xyuv_inimage = _mm_and_ps(_mm_cmpge_ps(xyuv_proj, img_lower_bound),
                                _mm_cmplt_ps(xyuv_proj, img_upper_bound));
      xyuv_inimage = _mm_and_ps(
          xyuv_inimage,
          _mm_shuffle_ps(
              xyuv_inimage, xyuv_inimage,
              _MM_SHUFFLE(2, 3, 0, 1))); // [x && y, y && x, u && v, v && u]
      xyuv_inimage =
          _mm_shuffle_ps(xyuv_inimage, xyuv_inimage,
                         _MM_SHUFFLE(0, 0, 2, 0)); // [x && y, u && v, ?, ?]

      // store warped depth
      _mm_storel_pi((__m64 *)warped_depth_ptr,
                    _mm_or_ps(_mm_and_ps(xyuv_inimage, zwzw),
                              _mm_andnot_ps(xyuv_inimage, NANS)));
      ...
      if ((xyuv_inimage_mask & 1) == 1) {
        ...
        __m128 xyxy_proj =
            _mm_shuffle_ps(xyuv_proj, xyuv_proj, _MM_SHUFFLE(1, 0, 1, 0));

        _mm_store_ss(warped_intensity_ptr + 0,
                     interpolateBilinearWithDepthBufferSse(
                         intensity_, depth_, xyxy_proj, zzzz, img_upper_bound));
      } else {
      ...
      if ((xyuv_inimage_mask & 2) == 2) {
        ...
        __m128 uvuv_proj =
            _mm_shuffle_ps(xyuv_proj, xyuv_proj, _MM_SHUFFLE(3, 2, 3, 2));

        _mm_store_ss(warped_intensity_ptr + 1,
                     interpolateBilinearWithDepthBufferSse(
                         intensity_, depth_, uvuv_proj, wwww, img_upper_bound));
      } else {
...
void RgbdImage::warpIntensitySse(const AffineTransform &transformation,
                                 const PointCloud &reference_pointcloud,
                                 const Eigen::Matrix3d &projection,
                                 RgbdImage &result,
                                 PointCloud &transformed_pointcloud) {
  warpIntensitySseImpl<WithPointCloud>(transformation, reference_pointcloud,
                                       projection, result,
...
void RgbdImage::warpIntensitySse(const AffineTransform &transformation,
                                 ...
                                 RgbdImage &result) {
  PointCloud tmp;
  warpIntensitySseImpl<WithoutPointCloud>(transformation, reference_pointcloud,
                                          projection, result, tmp);
}

void RgbdImage::calculateDerivativeYSseFloat(const cv::Mat &img,
                                             cv::Mat &result) {
  ...
  for (int x = 0; x < img.cols;
       x += inc, prev_ptr += inc, next_ptr += inc, result_ptr += inc) {
    _mm_store_ps(result_ptr, _mm_mul_ps(_mm_sub_ps(_mm_load_ps(next_ptr),
                                                   _mm_load_ps(prev_ptr)),
                                        scale));
    //_mm_stream_ps(result_ptr + 4, _mm_sub_ps(_mm_load_ps(next_ptr + 4),
    //_mm_load_ps(prev_ptr + 4)));
  }
