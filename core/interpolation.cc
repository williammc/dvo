...
IntensityType Interpolation::bilinear(const cv::Mat &img, float x, float y) {
  ...
  float interpolated = img.at<IntensityType>(y0, x0) * x_weight +
                       img.at<IntensityType>(y0, x1) * (1 - x_weight) +
                       img.at<IntensityType>(y1, x0) * x_weight +
                       img.at<IntensityType>(y1, x1) * (1 - x_weight) +
                       img.at<IntensityType>(y0, x0) * y_weight +
                       img.at<IntensityType>(y1, x0) * (1 - y_weight) +
                       img.at<IntensityType>(y0, x1) * y_weight +
                       img.at<IntensityType>(y1, x1) * (1 - y_weight);

  return (IntensityType)(interpolated * 0.25f);
}

IntensityType Interpolation::bilinearWithDepthBuffer(const cv::Mat &intensity,
                                                     const cv::Mat &depth,
                                                     const float &x,
                                                     const float &y,
                                                     const float &z) {
  ...
  if (std::isfinite(depth.at<float>(y0, x0)) &&
      depth.at<float>(y0, x0) > z_eps) {
    val += x0_weight * y0_weight * intensity.at<IntensityType>(y0, x0);
    sum += x0_weight * y0_weight;
  }

  if (std::isfinite(depth.at<float>(y0, x1)) &&
      depth.at<float>(y0, x1) > z_eps) {
    val += x1_weight * y0_weight * intensity.at<IntensityType>(y0, x1);
    sum += x1_weight * y0_weight;
  }

  if (std::isfinite(depth.at<float>(y1, x0)) &&
      depth.at<float>(y1, x0) > z_eps) {
    val += x0_weight * y1_weight * intensity.at<IntensityType>(y1, x0);
    sum += x0_weight * y1_weight;
  }

  if (std::isfinite(depth.at<float>(y1, x1)) &&
      depth.at<float>(y1, x1) > z_eps) {
