...
public:
  ...
  virtual void update(const Vector6 &J, const NumType &res,
                      const NumType &weight = 1.0f) = 0;
  virtual void update(const Eigen::Matrix<NumType, 2, 6> &J,
...
/// Basic interface for algorithms solving 1 step of non-linear least squares
/// where jacobians can be precomputed and don't vary between iterations.
...
public:
  ...
  virtual bool setResidualForConstraint(const size_t &idx, const NumType &res,
                                        const NumType &weight = 1.0f) = 0;
  virtual void finish() = 0;
  virtual void solve(Vector6 &x) = 0;
};
...
/// Builds normal equations and solves them with Cholesky decomposition.
class PrecomputedNormalEquationsLeastSquares
    : public PrecomputedLeastSquaresInterface {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
...
/// Same as NormalEquationsLeastSquares,
/// but solves normal equations with EigenValueDecomposition.
