...
void dvo::core::NormalEquationsLeastSquares::initialize(
...
void dvo::core::NormalEquationsLeastSquares::update(const dvo::core::Vector6 &J,
                                                    const NumType &res,
                                                    const NumType &weight) {
  NumType factor = weight; // weight * normalizer; // what happens without the
                           // normalizer? nothing!
  A_opt.rankUpdate(J, factor);
  // MathSse<Sse::Enabled, NumType>::addOuterProduct(A, J, factor);
  // A += J * J.transpose() * factor;
  MathSse<Sse::Enabled, NumType>::add(b, J,
                                      -res * factor); // not much difference :(
  // b -= J * res * factor;
...
void dvo::core::NormalEquationsLeastSquares::update(
    const Eigen::Matrix<NumType, 2, 6> &J,
...
void dvo::core::NormalEquationsLeastSquares::combine(
    const dvo::core::NormalEquationsLeastSquares &other) {
  A_opt += other.A_opt;
  b += other.b;
  // error += other.error;
...
void dvo::core::EvdLeastSquares::solve(dvo::core::Vector6 &x) {
  ...
  for (int i = 0; i < 6; ++i) {
    if (eigenvalues(i) < 0.05) {
      ...
      eigenvalues(i) = 1.0f / eigenvalues(i);
    }
  }
...
void dvo::core::SvdLeastSquares::update(const dvo::core::Vector6 &J,
                                        const NumType &res,
                                        const NumType &weight) {
...
dvo::core::PrecomputedNormalEquationsLeastSquares::
    ~PrecomputedNormalEquationsLeastSquares() {}

void dvo::core::PrecomputedNormalEquationsLeastSquares::initialize(
    const size_t maxnum_constraints) {
  hessian_.setZero();
...
void dvo::core::PrecomputedNormalEquationsLeastSquares::next() { ++mask_ptr_; }

void dvo::core::PrecomputedNormalEquationsLeastSquares::addConstraint(
    const size_t &idx, const dvo::core::Vector6 &J) {
...
void dvo::core::PrecomputedNormalEquationsLeastSquares::ignoreConstraint(
    const size_t &idx) {
  if ((*mask_ptr_) == 0)
    return;
...
bool dvo::core::PrecomputedNormalEquationsLeastSquares::
    setResidualForConstraint(const size_t &idx, const NumType &res,
                             const NumType &weight) {
  if (*mask_ptr_ == 0)
    return false;
...
void dvo::core::PrecomputedNormalEquationsLeastSquares::solve(
    dvo::core::Vector6 &x) {
  x = A.ldlt().solve(b);
}
