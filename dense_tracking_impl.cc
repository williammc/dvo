...
void computeResiduals(const PointIterator &first_point,
  ...
  Eigen::Matrix<float, 3, 4> KT =
      projection.cast<float>() * transform.matrix().block<3, 4>(0, 0);
  ...
  for (PointIterator p_it = first_point; p_it != last_point; ++p_it) {
    ...
    //  KT = K * Sophus::SE3::exp(xi + xi * t).matrix().block<3, 4>(0,
    //  0).cast<float>();
    //}

    // transformed_point = transform * p_it->getPointVec4f();

    // float projected_x = transformed_point(0) * projection(0, 0) /
    // transformed_point(2) + projection.ox();
    // float projected_y = transformed_point(1) * projection.fy() /
    // transformed_point(2) + projection.oy();
    ...
    if (!current.InImage(projected_x, projected_y) ||
        !current.InImage(projected_x + 1, projected_y + 1))
      continue;
    ...
    const float *x0y1_ptr =
        current.acceleration().ptr<float>(int(y0 + 1), int(x0));
    ...
    */
    Vector8f interpolated =
        (x0y0 * x0w + x1y0 * x1w) * y0w + (x0y1 * x0w + x1y1 * x1w) * y1w;

    if (!std::isfinite(interpolated(1)) || !std::isfinite(interpolated(4)) ||
        !std::isfinite(interpolated(5)))
      continue;

    // funny part: puzzling together depends on fwd. comp. / inverse comp. / esm
    // / additiv / ...
    // fwd. comp. for now
    result.last_point_error->getPointVec4f() =
        p_it->getPointVec4f(); // transformed_point;
    ...
    *result.last_residual =
        result.last_point_error->getIntensityAndDepthVec2f();
...
static const __m128 BLEND_MASK =
    _mm_cmpgt_ps(_mm_setr_ps(0.0f, 1.0f, 0.0f, 0.0f), _mm_set1_ps(0.5f));
...
void computeResidualsSse(
  ...
  const PointIterator lp =
      ((last_point - first_point) % 2) != 0 ? last_point - 1 : last_point;
  ...
  for (PointIterator p_it = first_point; p_it != lp; p_it += 2) {
    ...
    __m128 pt1_xy_pt2_xy =
        _mm_hadd_ps(_mm_hadd_ps(pt1_x, pt1_y), _mm_hadd_ps(pt2_x, pt2_y));
    __m128 pt1_zz_pt2_zz =
        _mm_hadd_ps(_mm_hadd_ps(pt1_z, pt1_z), _mm_hadd_ps(pt2_z, pt2_z));
    ...
    // check image bounds
    int bounds_mask =
        _mm_movemask_ps(_mm_and_ps(_mm_cmpge_ps(pt1_uv_pt2_uv, lower_bound),
                                   _mm_cmple_ps(pt1_uv_pt2_uv, upper_bound)));
    ...
    if ((bounds_mask & 3) == 3) {
      const float *x0y0_ptr =
          current.acceleration().ptr<float>(address[1], address[0]);
      const float *x0y1_ptr =
          current.acceleration().ptr<float>(address[1] + 1, address[0]);
#ifdef WIN32
      ...
      // interpolate
      __m128 a1 = _mm_mul_ps(
          w0_vvvv, _mm_add_ps(_mm_mul_ps(w0_uuuu, _mm_load_ps(x0y0_ptr + 0)),
                              _mm_mul_ps(w1_uuuu, _mm_load_ps(x0y0_ptr + 8))));

      __m128 b1 = _mm_mul_ps(
          w0_vvvv, _mm_add_ps(_mm_mul_ps(w0_uuuu, _mm_load_ps(x0y0_ptr + 4)),
                              _mm_mul_ps(w1_uuuu, _mm_load_ps(x0y0_ptr + 12))));

      __m128 a2 = _mm_mul_ps(
          w1_vvvv, _mm_add_ps(_mm_mul_ps(w0_uuuu, _mm_load_ps(x0y1_ptr + 0)),
                              _mm_mul_ps(w1_uuuu, _mm_load_ps(x0y1_ptr + 8))));

      __m128 b2 = _mm_mul_ps(
          w1_vvvv, _mm_add_ps(_mm_mul_ps(w0_uuuu, _mm_load_ps(x0y1_ptr + 4)),
      ...
      if (nans_mask == 0) {
        ...
        reference_a = _mm_or_ps(_mm_and_ps(BLEND_MASK, pt1_zz_pt2_zz),
                                _mm_andnot_ps(BLEND_MASK, reference_a));

        __m128 residual_a =
            _mm_add_ps(_mm_mul_ps(current_weight_a, a),
                       _mm_mul_ps(reference_weight_a, reference_a));
        _mm_store_ps(result.last_point_error->intensity_and_depth.data + 0,
                     residual_a);

        // occlusion test
        if (result.last_point_error->intensity_and_depth.z >
            -20.0f * depthStdDevZ((p_it + 0)->intensity_and_depth.z)) {
          _mm_storel_pi((__m64 *)result.last_residual->data(), residual_a);

          __m128 reference_b =
              _mm_load_ps((p_it + 0)->intensity_and_depth.data + 4);
          __m128 residual_b =
              _mm_add_ps(_mm_mul_ps(current_weight_b, b),
                         _mm_mul_ps(reference_weight_b, reference_b));
    ...
    if ((bounds_mask & 12) == 12) {
      const float *x0y0_ptr =
          current.acceleration().ptr<float>(address[3], address[2]);
      const float *x0y1_ptr =
          current.acceleration().ptr<float>(address[3] + 1, address[2]);
      ...
      if (nans_mask == 0) {
        ...
        // replace the reference image depth value with the transformed one
        reference_a =
            _mm_or_ps(_mm_and_ps(BLEND_MASK,
                                 _mm_unpackhi_ps(pt1_zz_pt2_zz, pt1_zz_pt2_zz)),
                      _mm_andnot_ps(BLEND_MASK, reference_a));
        ...
        // occlusion test
        if (result.last_point_error->intensity_and_depth.z >
            -20.0f * depthStdDevZ((p_it + 1)->intensity_and_depth.z)) {
          _mm_storel_pi((__m64 *)result.last_residual->data(), residual_a);

          __m128 reference_b =
              _mm_load_ps((p_it + 1)->intensity_and_depth.data + 4);
          __m128 residual_b =
              _mm_add_ps(_mm_mul_ps(current_weight_b, b),
                         _mm_mul_ps(reference_weight_b, reference_b));
          _mm_store_ps(result.last_point_error->intensity_and_depth.data + 4,
...
void computeResidualsSse(
    ...
    const Eigen::Affine3f &transform, const Vector8f &reference_weight,
    const Vector8f &current_weight, ComputeResidualsResult &result) {
  computeResidualsSse<false>(first_point, last_point, current, projection,
                             transform, reference_weight, current_weight,
                             result);
}
...
void computeResidualsAndValidFlagsSse(
    ...
    const Eigen::Affine3f &transform, const Vector8f &reference_weight,
    const Vector8f &current_weight, ComputeResidualsResult &result) {
  computeResidualsSse<true>(first_point, last_point, current, projection,
                            transform, reference_weight, current_weight,
                            result);
}
...
float computeCompleteDataLogLikelihood(const ResidualIterator &first_residual,
  ...
  for (auto err_it = first_residual; err_it != last_residual; ++err_it, ++c) {
    error_acc *=
        (1.0 + 0.2 * ((*err_it).transpose() * precision * (*err_it))(0, 0));
  ...
  return 0.5 * n * std::log(precision.determinant()) -
         0.5 * (5.0 + 2.0) * error_sum;
}

static inline float computeWeightedErrorPart(const float &weight,
                                             const Eigen::Vector2f &r,
                                             const Eigen::Matrix2f &precision) {
...
float computeWeightedError(const ResidualIterator &first_residual,
  ...
  for (auto err_it = first_residual; err_it != last_residual;
       ++err_it, ++w_it) {
...
float computeWeightedErrorSse(const ResidualIterator &first_residual,
                              ...
                              const WeightIterator &first_weight,
                              const Eigen::Matrix2f &precision) {
  const ResidualIterator lr =
      last_residual - ((last_residual - first_residual) % 4);
  WeightIterator w_it = first_weight;

  __m128 error_acc = _mm_setzero_ps();
  __m128 prec = _mm_load_ps(precision.data());

  for (auto err_it = first_residual; err_it != lr; err_it += 4, w_it += 4) {
    ...
    __m128 dist_r1r2 = _mm_mul_ps(
        _mm_hadd_ps(_mm_mul_ps(diff_r1r1, prec), _mm_mul_ps(diff_r2r2, prec)),
        diff_r1r2);
    ...
    __m128 dist_r3r4 = _mm_mul_ps(
        _mm_hadd_ps(_mm_mul_ps(diff_r3r3, prec), _mm_mul_ps(diff_r4r4, prec)),
        diff_r3r4);
  ...
  for (auto err_it = lr; err_it != last_residual; ++err_it, ++w_it) {
    weighted_error += computeWeightedErrorPart(*w_it, *err_it, precision);
  }
...
Eigen::Vector2f computeMean(const ResidualIterator &first_residual,
                            const ResidualIterator &last_residual,
                            const WeightIterator &first_weight) {
  WeightIterator w_it = first_weight;
...
Eigen::Vector2f computeMeanSse(const ResidualIterator &first_residual,
                               const ResidualIterator &last_residual,
                               const WeightIterator &first_weight) {
  ...
  for (auto err_it = first_residual; err_it != lr;
       err_it += 4, weights_ptr += 4) {
    ...
    residual_acc =
        _mm_add_ps(residual_acc,
                   _mm_add_ps(_mm_mul_ps(_mm_unpacklo_ps(w1234, w1234), r12),
                              _mm_mul_ps(_mm_unpackhi_ps(w1234, w1234), r34)));
  }
  ...
  for (auto err_it = lr; err_it != last_residual; ++err_it, ++weights_ptr) {
...
static inline Eigen::Matrix2f computeScalePart(const float &weight,
                                               const Eigen::Vector2f &r,
                                               const Eigen::Vector2f &mean) {
  Eigen::Vector2f diff;
  diff = r - mean;
...
Eigen::Matrix2f computeScaleSse(const ResidualIterator &first_residual,
                                ...
                                const Eigen::Vector2f &mean) {
  const ResidualIterator lr =
      last_residual - ((last_residual - first_residual) % 2);
  ...
  __m128 fac1, fac2, w;
  for (auto err_it = first_residual; err_it != lr; err_it += 2, w_it += 2) {
...
static inline float computeWeight(const Eigen::Vector2f &r,
                                  const Eigen::Vector2f &mean,
                                  const Eigen::Matrix2f &precision) {
...
void computeWeightsSse(const ResidualIterator &first_residual,
  ...
  for (auto err_it = first_residual; err_it != lr; err_it += 4, w_ptr += 4) {
    __m128 r1r2 = _mm_load_ps((err_it + 0)->data());
    ...
    _mm_store_ps(w_ptr,
                 _mm_mul_ps(six, _mm_rcp_ps(_mm_add_ps(five, dist_r1r2r3r4))));
  }

  for (auto err_it = lr; err_it != last_residual; ++err_it, ++w_ptr) {
...
void computeMeanScaleAndWeights(const ResidualIterator &first_residual,
  ...
  do {
    ...
    Eigen::Matrix2f S2 =
        computeScaleSse(first_residual, last_residual, first_weight, mean);
    precision = S2.inverse();

    computeWeightsSse(first_residual, last_residual, first_weight, mean,
                      precision);

    // mean = computeMeanSse(first_residual, last_residual, first_weight);

    convergence_criterion =
        (last_precision - precision).lpNorm<Eigen::Infinity>();
    iterations += 1;
